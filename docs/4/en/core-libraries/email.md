---
title: Email
keywords: "sending mail,email sender,envelope sender,php class,database configuration,sending emails,shells,smtp,transports,attributes,array,config,flexibility,php email,new email,sending email,models"
---
...
- `'transport'`: Transport configuration name. See [email-transport](email.md#email-transport).
- `'log'`: Log level to log the email headers and message. `true` will use
  LOG_DEBUG. See [logging-levels](logging.md#logging-levels). Note that logs will be emitted under the scope named `email`.
  See also [logging-scopes](logging.md#logging-scopes).
