...
#### Static Method `Cake\Cache\Cache::config($key, $config = null)`
...
<!-- anchor: cache-configuration-fallback -->
...
#### Static Method `Cake\Cache\Cache::clear($check, $config = 'default')`
...
#### Static Method `Cake\Cache\Cache::gc($config)`
