...
#### Static Method `rijndael($text, $key, $mode)`
...
#### Static Method `encrypt($text, $key, $hmacSalt = null)`
...
#### Static Method `hash( $string, $type = NULL, $salt = false )`
