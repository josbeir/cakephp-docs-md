...
#### Method `currency(float $number, string $currency = 'USD', array $options = array())`
...
#### Method `defaultCurrency(string $currency)`

:param string $currency: Set a known currency for `CakeNumber::currency()`.
...
> [!IMPORTANT]
> Added in version 2.3 This method was added in 2.3
>

#### Method `addFormat(string $formatName, array $options)`
...
#### Method `precision(mixed $number, int $precision = 3)`
...
#### Method `toPercentage(mixed $number, int $precision = 2, array $options = array())`
...
#### Method `fromReadableSize(string $size, $default)`
...
#### Method `toReadableSize(string $dataSize)`
...
#### Method `format(mixed $number, mixed $options=false)`
...
#### Method `formatDelta(mixed $number, mixed $options=array())`
