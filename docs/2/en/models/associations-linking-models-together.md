---
title: "Associations: Linking Models Together"
keywords: "relationship types,relational mapping,recipe database,relational database,this section covers,web applications,recipes,models,cakephp,storage"
---
...
[plugin-models](../plugins/how-to-create-plugins.md#plugin-models).
...
| many to many | hasAndBelongsToMany | Recipes have, and belong to, many ingre |
...
by defining an array. See [multiple-counterCache](associations-linking-models-together.md#multiple-counterCache).
...
| --- | --- |
| Recipe HABTM Ingredient | **ingredients_recipes**.id, **ingredients_recipes**.ingredient_i |
...
assumed, they must be changed in model's [model-primaryKey](model-attributes.md#model-primaryKey).
...
HABTM, see [hasMany-through](associations-linking-models-together.md#hasMany-through).

> [!TIP]
> For more information on saving HABTM objects, see [saving-habtm](saving-your-data.md#saving-habtm)

<!-- anchor: hasMany-through -->
...
key in [HABTM association arrays](associations-linking-models-together.md#ref-habtm-arrays).
...
If your table has a `parent_id` field, you can also use [model-find-threaded](retrieving-your-data.md#model-find-threaded)
to fetch a nested array of records using a single query without
setting up any associations.
<!-- anchor: joining-tables -->
### Joining tables
