...
`method` Cake\\View\\Helper\\HtmlHelper(View::**scriptBlock**(string $code, array $options = [])
...
`method` Cake\\View\\Helper\\HtmlHelper(View::**scriptStart**(array $options = [])
...
`method` Cake\\View\\Helper\\HtmlHelper(View::**importmap(array $map, array $options = []): string**()
